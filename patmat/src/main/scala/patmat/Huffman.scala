...
object Huffman extends HuffmanInterface {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    ...
    def timeAbr(charList: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] = charList match {
      case Nil => acc
      case x :: xs => timeAbr(xs.filter(_ != x), (x, xs.count(_ == x)) :: acc)
    }
  ...
  def makeOrderedLeafList(freq: List[(Char, Int)]): List[Leaf] = {
    freq.sortBy(x => x._2).map(x => Leaf(x._1, x._2))
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    ...
    //    case Nil => trees
    //    case x :: Nil => trees
    //    case a :: b :: xs => (makeCodeTree(a, b) :: trees.tail.tail).sortBy(weight)
  ...
  def until(done: List[CodeTree] => Boolean, merge: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
    if (done(trees)) trees
    else until(done, combine)(merge(trees))
  }
